...
class ProductController extends Controller
{
    ...
    public function create(){
        $categories = Category::where('estatus',1)->where('parent_category_id',0)->get()->toArray();
        $catArray = array();
        foreach ($categories as $category){
            if( empty(getSubCategories($category['id'])) ){
                $category['subcategory'] = null;
            }
            else{
                $category['subcategory'] = getSubCategories($category['id']);
            }
...
//            $VariantCnt = 1;
        ...
        if (isset($required_specifications) && !empty($required_specifications)){
            ...
            foreach ($required_specifications as $required_specification){
                $html_required_specification.= '<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12">
                ...
                $html_required_specification .= '<select class="form-control specReq" id="" name="specReq'.$required_specification['id'].'">';
        ...
        if (isset($optional_specifications) && !empty($optional_specifications)){
            ...
            foreach ($optional_specifications as $optional_specification){
                $html_optional_specification.= '<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12">
                                                    <div class="form-group row">
                ...
                $html_optional_specification .= '<select class="form-control specOpt" data-id="'.$optional_specification['id'].'" name="specOpt'.$optional_specification['id'].'">';
        ...
        $html .= '</label>';
        $html .= '</div>';
        $html .= '<div class="col-lg-2 col-sm-2 actionbox ml-auto"><a role="button" class="collapse-arrow variantbox-collapse" data-toggle="collapse" href="#" aria-expanded="true" onclick="collapsePanel(this)"></a><div id=""></div></div>';
        $html .= '</div>';
...
//        $myValue = array();
//        parse_str($request['variantForm1'],$myValue);
...
//        dd($variants_status);
        ...
        for ($i=1;$i<=count($attr_term_ids);$i++) {
            ...
            $product_variant->product_title = $myValue['varProductName'];
            ...
            if(isset($myValue['varSpecRequired'])) {
                ...
                foreach ($varSpecRequired as $specReq) {
                    ...
                    $product_variant_specification->attribute_term_id = $myValue['specReq' . $specReq];
            ...
            if (isset($myValue['varSpecOptional']) && $myValue['varSpecOptional']!="") {
                ...
                foreach ($varSpecOptional as $specOpt) {
                    ...
                    $product_variant_specification->attribute_term_id = $myValue['specOpt'.$specOpt];
...
//        dd($product_variant_old_images,$not_removable_images);
        if($request->action=="addProduct") {
            $cat_id = isset($category_ids[2]) ? $category_ids[2] : $category_ids[1];
...
//        dd($request->all());
        if ($request->ajax()) {
            ...
            {
                $products = ProductVariant::with('product.primary_category','product.child_category','product.sub_child_category','product.attribute','attribute_term')
                    ->offset($start)
                    ->limit($limit)
                    ->orderBy($order,$dir)
                    ->get();
                ...
                $products =  ProductVariant::with('product.primary_category','product.child_category','product.sub_child_category','product.attribute','attribute_term')
                    ...
                    ->orWhereHas('product',function ($mainQuery) use($search) {
                        $mainQuery->where('hsn_code', 'Like', '%' . $search . '%');
                    })
            ...
            if(!empty($products))
            {
                foreach ($products as $product)
                {
                    ...
                    if (!isset($product->product->sub_child_category)) {
                        $categories = $product->product->primary_category->category_name . " / " . $product->product->child_category->category_name;
                    }
                    else{
                        $categories = $product->product->primary_category->category_name . " / " . $product->product->child_category->category_name . " / " . $product->product->sub_child_category->category_name;
                    }

                    if (isset($product->product->hsn_code)){
                        $product_info = '<span>'.$product->product_title.'</span><span>HSN: '.$product->product->hsn_code.'</span><span>Variation Title: '.$product->product->attribute->attribute_name.'</span><span>Variation Value: '.$product->attribute_term->attrterm_name.'</span>';
                    }
                    else{
                        $product_info = '<span>'.$product->product_title.'</span><span>Variation Title: '.$product->product->attribute->attribute_name.'</span><span>Variation Value: '.$product->attribute_term->attrterm_name.'</span>';
                    }
                    ...
                    $nestedData['image'] = '<img src="'.url('public/'.$images[0]).'" width="50px" height="50px"/>';
...
//            return json_encode($json_data);
    ...
    public function editproduct($id){
        ...
        $product = Product::with('primary_category','child_category','sub_child_category','attribute.attributeterm','product_variant.product_variant_specification')->where('id',$id)->first();

        $attr_term_ids = explode(",",$product->attr_term_ids);
//        dd($product->toArray());
        $CategorySel = isset($product->subchild_category_id) ? ($product->primary_category->category_name." / ".$product->child_category->category_name." / ".$product->sub_child_category->category_name) : ($product->primary_category->category_name." / ".$product->child_category->category_name);
